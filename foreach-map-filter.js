...
function vowelCount(str){
   ...
   for(char of lowerCaseStr){
    if('aeiou'.indexOf(char)!==-1){
        newObject[char]=newObject[char]? newObject[char] + 1 : 1;
    }
   }
   return newObject;
}
...
function extractFullName(arr){
    return arr.map((v)=>{return v.first+" "+v.last;});
}
...
function filterByValue(arr, key) {
    return arr.filter((v)=>{return v[key];})
}
...
function find(arr, searchValue) {
    return arr.filter((v)=>{return v===searchValue;})[0];
}
...
function findInObj(arr, key, searchValue) {
    return arr.filter((v)=>{return v[key]===searchValue;})[0];
}
